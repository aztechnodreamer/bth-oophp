...
/**
 * Check slug and make unique.
 *
 * @param string $str the slug as string.
 * @param object $db databaseconnection.
 *
 * @return str the unique slug.
 */
function checkSlug($contentSlug, $contentId, $db)
{
    $sql = "SELECT slug FROM content WHERE slug = ? AND id != ?;";
    $res = $db->executeFetchAll($sql, [$contentSlug, $contentId]);

    if (sizeof($res) != 0) {
        do {
            if (is_numeric(substr($contentSlug, -1))) {
                $contentSlug = substr_replace($contentSlug, (substr($contentSlug, -1) + 1), -1);
            } else {
                $contentSlug .= 2;
            }
            $sql = "SELECT slug FROM content WHERE slug = ? AND id != ?;";
